...
public class TokenDetector {
	...
	// Capture et enregistrement du fond
    ...
    public Point detectTokenWithCanny(Mat frame) {
        ...
        if (centerPoint != null) {
            //Imgproc.circle(frame, centerPoint, 5, new Scalar(255, 0, 0), -1); // Draw the center point
            return centerPoint;
	...
	// Affichage du score sur la frame
	public Mat displayHud(Mat frame, int scoreGameBlue, int scoreGameRed, int advantage) {
		...
		Imgproc.putText(frame, "Blue Team : " + scoreGameBlue, blueTeamPosition, fontFace, fontScale, blueColor,
				thickness);
	...
	public Mat displayTarget(Mat frame, Point target) {
		...
		Imgproc.line(frame, new Point(target.x - crossSize, target.y), new Point(target.x + crossSize, target.y),
				greenColor, thickness);

		Imgproc.line(frame, new Point(target.x, target.y - crossSize), new Point(target.x, target.y + crossSize),
				greenColor, thickness);
